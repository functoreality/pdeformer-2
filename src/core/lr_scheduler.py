...
def get_lr_list(steps_per_epoch: int,
    ...
    This function supports 'multi_step' or 'cosine_decay' learning rate
    schedulers, and can also perform warmup before the learning rate scheduler.

    Args:
        steps_per_epoch (int): The number of steps per epoch.
        epochs (int): The total number of epochs.
        lr_init (float): The initial learning rate.
        lr_scheduler_type (str): The type of learning rate scheduler. It should
            be one of 'multi_step', 'cosine_decay', 'warmup_cosine_decay', or
            'warmup_multi_step'.
        lr_milestones (list, optional): A list of milestones at which the
            learning rate is reduced. Default: [0.6, 0.8, 1.0].
        lr_decay (float): The factor by which the learning rate is decayed at
            each milestone. Default: 0.2.
        enable_warmup (bool): Whether to enable warmup. Default: False.
        warmup_epochs (int): The number of epochs over which the learning rate
            warms up to `lr_init`. Default: 0.
    ...
    if lr_scheduler_type in ['piecewise_constant', 'multi_step', 'mstep']:
        ...
        raise ValueError(
            ...
            "{'piecewise_constant', 'multi_step', 'mstep', 'cosine_decay', "
            f"'cos'}}, but got {lr_scheduler_type}")
